-- Diagnostik: notify_user_push schreibt jetzt einen Trace-Log am Ende
-- damit wir sehen ob die Funktion ueberhaupt durchlaeuft.
...
begin
    ...
    values (p_user_id, 'push', 'skipped', 'TRACE_enter', p_title || ' [' || p_channel || ']', '{"diag":"enter"}'::jsonb);
    ...
    if coalesce(v_recipient_is_test, false) = false
       and coalesce(v_actor_is_test, false) = true then
        ...
        values (p_user_id, 'push', 'skipped', 'is_test_actor_blocked', p_title);
        return;
    end if;
    ...
    if coalesce(p_ref_id, p_data->>'ref_id', '') <> ''
       and public.is_duplicate_push(p_user_id, coalesce(p_ref_id, p_data->>'ref_id')) then
        insert into public.notification_logs (user_id, channel, status, error_code, title)
    ...
    -- pg_net Call mit Capture der request_id fuer Diagnose
    ...
    insert into public.notification_logs (user_id, channel, status, error_code, title, metadata)
    values (p_user_id, 'push', 'skipped', 'TRACE_pg_net_fired', p_title,
            jsonb_build_object('pg_net_id', v_pg_net_id, 'url', v_url));
